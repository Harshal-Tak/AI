def aStarAlgo(start_node,stop_node):
    ...
    parents = {} #parent contains adjacency map of all nodes
    ...
    while len(open_set)>0:
        ...
        if n == stop_node or Graph_nodes[n] == None:
            ...
            for (m,weight) in get_neighbours(n):
                ...
                #for each node m, compare its distance from start i.e. g(n) to the
                #from start node through n node
                else:
                    if g[m] > g[n] + weight:
                        #update g(n)
                        g[m] = g[n] + weight
...
#define function to retrieve neighbour and its distance from the passed node
...
#for simplicit we'll consider heuristic distances given
#and this function returns heuristic distance for all nodes
def heuristic(n):
    H_dist = {'A':11,'B':6,'C':99,'D':5,'E':7,'G':0}
    return H_dist[n]
